  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-fade-in">
      <div 
        ...
        className="w-full max-w-lg bg-surface text-text-primary rounded-2xl border border-border-hairline shadow-2xl p-6 space-y-5 animate-scale-in"
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-3 border-b border-border-hairline">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-accent-sage-tint text-accent-sage flex items-center justify-center">
              <BookMarked className="w-4 h-4" />
            </div>
            <div>
              <h3 className="font-serif-heading text-lg font-bold text-text-primary">Save to Notebook</h3>
              <p className="text-[13px] text-text-muted">Curate into your high-signal insight collection</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 text-text-muted hover:text-text-primary rounded-md transition-colors cursor-pointer"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        {/* Excerpt preview */}
        <div className="p-3.5 bg-canvas rounded-xl border border-border-hairline text-xs text-text-primary leading-relaxed max-h-32 overflow-y-auto italic font-serif">
          "{excerpt}"
        </div>
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            {isGeneratingHint && (
              <span className="text-[13px] text-text-muted flex items-center gap-1">
                <RefreshCw className="w-2.5 h-2.5 animate-spin text-accent-sage" />
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full p-2.5 text-xs bg-surface text-text-primary border border-border-hairline rounded-lg focus:outline-none focus:ring-2 focus:ring-accent-sage/20 focus:border-accent-sage resize-none"
          />
        </div>
        ...
        <div className="flex items-center justify-end gap-2 pt-2 border-t border-border-hairline">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-accent-sage hover:opacity-90 rounded-lg transition-colors shadow-2xs cursor-pointer"
          >
